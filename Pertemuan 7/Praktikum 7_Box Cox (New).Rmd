...
1. Nilai harapan galat sama dengan nol ($E[ε_i]=0$)

2. Homoskedastisitas atau ragam galat homogen ($Var[ε]=σ^2I$)

3. Autokorelasi atau galat saling bebas ($E[ε_i,ε_j]=0$)

4. Galat menyebar Normal ($ε−N(0,σ^2I)$)
...
$$\hat{Y}= 1807 + 0.0362 X1 + 45.08 X2 + 0.0012 X3$$
...
Nilai p pada uji shapiro wilk sama dengan 0.1336 yang lebih besar dari alpha 5%, tetapi dengan lilliefors menghasilkan nilai p 0.0101 yang kurang dari $α=0.05$. Pada latihan kali ini, hasil liliefors yang digunakan agar bisa diterapkan penanganan asumsi, sehingga dinyatakan asumsi normalitas sisaan tidak terpenuhi.
...
```{r}
bc_model <- boxcox(Income ~ Population + `HS Grad` + Area, data = data,lambda = seq(-2, 2, by = 0.1))
```
...
\hline
\textbf{Sumber Keragaman} & \textbf{Derajat Bebas (db)} & \textbf{Jumlah Kuadrat (JK)} & \textbf{Kuadrat Tengah (KT)} & \textbf{$F_{\text{hitung}}$} & \textbf{$P\text{-value}$} \\ \hline
Regresi & 3 & 0.44686 & 0.14895 & 3.5115 & 0.02927 \\
Sisaan & 26 & 1.10280 & 0.04242 & & \\
Total & 29 & 1.54966 & & & \\ \hline
\end{tabular}
\end{table}
